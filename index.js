...
const séries         = JSON.parse(fs.readFileSync(path.join(dossierSaison, 'séries.json'), 'utf-8'));
const questionnaires = JSON.parse(fs.readFileSync(path.join(dossierSaison, 'questionnaires.json'), 'utf-8'));
...
app.post('/api/joueurs/nouveau', (req, res) => {
    ...
    const joueursÉquipe = joueurs.filter(j => j.noÉquipe === noÉquipe && !j.estÉquipe);
    ...
    fs.writeFileSync(
        ...
        JSON.stringify(joueurs, null, 2), 'utf-8'
    );
...
function reculerQuestion(état) {
    if (état.noQuestionActuelle > 1) {
        ...
        const indexActuel = séries.findIndex(s => s.noSérie === état.noSérieActuelle);
        if (indexActuel > 0) {
...
// ============================================================
function enregistrerRépondant(état, répondants, cheminPartie, { noÉquipe, noJoueur, points }) {
    ...
    console.log(`✅ ${points} pts → équipe ${noÉquipe} (partie ${état.noPartie})`);
}
...
io.on('connection', (socket) => {
    ...
    socket.on('sEnregistrer', (joueur) => {
        ...
        io.to(`partie-${joueur.noPartie}`).emit('joueursConnectés', état.joueursConnectés);
    });
    ...
    socket.on('choisirPartie', (noPartie) => {
            ...
            try {
                ...
                if (répondants.length > 0) {
                    ...
                    répondants.forEach(r => {
                        console.log(`Traitement: équipe ${r.noÉquipe}, série ${r.noSérie}, Q${r.noQuestion}`);
                        ...
                        console.log(`Répondant: équipe ${r.noÉquipe}, série ${r.noSérie}, Q${r.noQuestion}, points calculés: ${points}`);
                    ...
                    const dernier = répondants.slice().sort((a, b) =>
                        a.noSérie !== b.noSérie
                            ? a.noSérie - b.noSérie
        ...
        if (reprise) {
            ...
            io.to(`partie-${noPartie}`).emit('questionMisÀJour', questionCourante(état));
        }
    ...
    socket.on('attribuerPoints', ({ noPartie, noÉquipe, noJoueur, points }) => {
        ...
        // Vérifier doublon
        const indexDoublon = répondants.findIndex(r =>
            r.noSérie === état.noSérieActuelle &&
            r.noQuestion === état.noQuestionActuelle
        );
        ...
        enregistrerRépondant(état, répondants, cheminPartie, { noÉquipe, noJoueur, points });
    });
    ...
    // --------------------------------------------------------
    socket.on('confirmerÉcrasement', ({ noPartie, noÉquipe, noJoueur, points }) => {
        const état = obtenirÉtat(noPartie);
        const cheminPartie = path.join(dossierSaison, 'parties',
            `répondants-${noPartie}.json`);
    ...
    socket.on('mauvaiseRéponse', ({ noPartie, noÉquipe }) => {
        ...
        if (série.réplique) {
            ...
            const partie = parties.find(p => p.noPartie === noPartie);
            état.réplique = (noÉquipe === partie.noÉquipeA)
                ? partie.noÉquipeB
                : partie.noÉquipeA;
            état.buzzVerrou = false;
            io.to(`partie-${noPartie}`).emit('réplique', état.réplique);
            console.log(`↩️ Droit de réplique → équipe ${état.réplique} (partie ${noPartie})`);
        } else {
    ...
    socket.on('terminerPartie', (noPartie) => {
        ...
        try {
            ...
            try {
                const contenuCumulatif = fs.readFileSync(cheminCumulatif, 'utf-8').trim();
                répondantsCumulatifs = contenuCumulatif ? JSON.parse(contenuCumulatif) : [];
            } catch (e) {
            ...
            fs.writeFileSync(cheminCumulatif, JSON.stringify(fusion, null, 2), 'utf-8');

            console.log(`✅ Partie ${noPartie} fusionnée — ${répondantsPartie.length} répondants`);
